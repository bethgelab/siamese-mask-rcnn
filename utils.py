...
def display_siamese_instances_teaser(target, image, boxes, masks, class_ids,
                      scores=None, title="",
                      figsize=(16, 16), ax=None,
                      show_mask=True, show_bbox=True,
    ...
    if not ax:
        ...
        gs = GridSpec(4, 5)
        ax = plt.subplot(gs[:, 3:])
        ax_input = plt.subplot(gs[:,:2])
        target_ax = plt.subplot(gs[1, 2])
        auto_show = True
    ...
    ax.set_xlim(0, width )
    ax.axis('off')
    ax.set_title('Output', fontsize=fontsize)

    ax_input.set_ylim(height, 0)
    ax_input.set_xlim(0, width)
    ax_input.axis('off')
    ax_input.set_title('Input image', fontsize=fontsize)
    ax_input.imshow(image)
    
    target_height, target_width = target.shape[:2]
    target_ax.set_ylim(target_height, 0)
    target_ax.set_xlim(0, target_width)
    target_ax.axis('off')
    target_ax.set_title('Target', fontsize=fontsize)
