...
def display_siamese_instances2(target_list, image_list, boxes_list, masks_list, class_ids_list,
    ...
    if not ax:
        ...
        # Use GridSpec to show target smaller than image
        fig = plt.figure(figsize=figsize)
        gs = GridSpec(M, M, hspace=0.02, wspace=0.03, left=0, right=1, bottom=0, top=1)
        auto_show = True
    ...
    for m1 in range(M):
        for m2 in range(M):
            ...
            rect = visualize.patches.Rectangle((target_shift, height - target_shift), target_width * 2, -target_height * 2, linewidth=2, edgecolor='white', facecolor='none', zorder=10)
            ax.add_patch(rect)
            index = index + 1
...
def display_siamese_instances_teaser(target_list, image_list, boxes_list, masks_list, class_ids_list,
                      scores_list=None, title="",
                      figsize=(16, 16), ax=None,
                      show_mask=True, show_bbox=True,
                      colors=None, captions=None, fontsize=20,
                      filename='teaser.svg'):
    """
    boxes: [num_instance, (y1, x1, y2, x2, class_id)] in image coordinates.
    masks: [height, width, num_instances]
    ...
    if not ax:
        ...
        ax_input = plt.subplot(gs[:,3:5])
        target_ax1 = plt.subplot(gs[1, 2])
        target_ax2 = plt.subplot(gs[1, 5])
        auto_show = True

        axs = [ax1, ax2]
        target_axs = [target_ax1, target_ax2]
    ...
    ax1.axis('off')
    ax1.set_title('Output', fontsize=fontsize, y=1.01)

    ax2.set_ylim(height, 0)
    ax2.set_xlim(0, width )
    ax2.axis('off')
    ax2.set_title('Output', fontsize=fontsize, y=1.01)

    ax_input.set_ylim(height, 0)
    ax_input.set_xlim(0, width)
    ax_input.axis('off')
    ax_input.set_title('Input image', fontsize=fontsize, y=1.01)
    ax_input.imshow(image_list[0])
    
    target_height, target_width = target_list[0].shape[:2]
    target_ax1.set_ylim(target_height, 0)
    target_ax1.set_xlim(0, target_width)
    target_ax1.axis('off')
    target_ax1.set_title('Target', fontsize=fontsize, y=1.05)

    target_height, target_width = target_list[1].shape[:2]
    target_ax2.set_ylim(target_height, 0)
    target_ax2.set_xlim(0, target_width)
    target_ax2.axis('off')
    target_ax2.set_title('Target', fontsize=fontsize, y=1.05)

    j = 0
    for ax, target_ax in zip(axs, target_axs):
        target = target_list[j]
        image = image_list[j]
        boxes = boxes_list[j]
        masks = masks_list[j]
        class_ids = class_ids_list[j]
